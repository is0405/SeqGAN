...
def load_text(filename):
    ...
    dic = json.load(json_open)

    sequence = dic[0]['sequence']
    return sequence

def wakati(filetext):
    tagger = MeCab.Tagger()
    result = tagger.parse(filetext)
    return result
...
def main():
    raw_text = load_text('save/data_output.json')
    cleaned_text = clean_text(wakati(raw_text))
    ...
    print(generate_text(model, char2id, id2char, '婚約'))
